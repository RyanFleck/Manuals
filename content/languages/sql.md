...
aliases: ["/sql", "/sequel", "/tsql", "/t-sql", "/psql", "/postgres", "/SQL"]
---
...
## TABLE - CREATE, ALTER, DROP {#table---create-alter-drop}
...
## INDEX - CREATE, ALTER, DROP {#index---create-alter-drop}
...
## SELECT WHERE NULL/NOT NULL {#select-where-nullnot-null}
...
## SELECT WHERE BETWEEN {#select-where-between}
...
## SELECT WHERE Combinations - AND, OR {#select-where-combinations---and-or}
...
{{< figure src="/images/sql_joins.png?invert=true&noborder=true" caption="<span class=\"figure-number\">Figure 1: </span>Overview of SQL joins by C.L. Moffatt from [codeproject.com](https://www.codeproject.com/articles/33052/visual-representation-of-sql-joins), which includes excellent explanations for each diagram as well [(format howto)](https://www.instructables.com/How-to-Remove-the-White-Background-From-Images-si/)" class="noborder invert" >}}
...
### LEFT/RIGHT JOIN EXCLUDING INNER JOIN {#leftright-join-excluding-inner-join}
...
### OUTER JOIN EXCLUDING INNER JOIN {#outer-join-excluding-inner-join}
...
row in table B. This is known as a _Cartesian product_ - it returns
every possible combination of rows. Cross joins are useful for
_reducing_ a table to come to conclusions.
...
## UNION (Combine Similar Tables) {#union-combine-similar-tables}
...
## WITH (Provide a Virtual Table) {#with-provide-a-virtual-table}
...
-   [COALESCE](https://neon.tech/postgresql/postgresql-tutorial/postgresql-coalesce)
...
## PostGIS - Utilizing Positional Data {#postgis---utilizing-positional-data}
...
# Other Engine-Specific Notes {#other-engine-specific-notes}
...
# SQLite Code of Ethics {#sqlite-code-of-ethics}
